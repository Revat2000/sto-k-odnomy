      ...
      <v-card flat>
        <v-col cols="12" class="py-0">
          <v-card-title class="text-h6 px-0 justify-center"
            >КОМАНДА 1</v-card-title
          >
          <!-- points avatar -->
          <v-avatar color="orange" size="62" rounded class="mr-2 mb-3">
            <span class="white--text headline" v-text="teamFirstPoints"></span>
          </v-avatar>
        ...
        <v-card-actions class="py-0 pl-0">
          <v-col cols="12">
            <v-btn color="warning" fab dark @click="pointsDistribut('first')">
...
export default {
  ...
  watch: {
    ...
    nextRound() {
      ...
      if (this.$store.getters.getNextRound) {
        ...
        this.$store.dispatch("CLEAR_TEAM_ERROR", "first");
      }
    },
  },
  ...
  methods: {
    ...
    async pointsDistribut(val) {
      ...
      this.teamFirstPoints = this.$store.getters.getFirstTeamScore;
    ...
    async setErrorArr() {
      ...
      await this.$store.dispatch("CLEAR_TEAM_ERROR", "first");
      const curErrArr = this.toggle_exclusive;
      await this.$store.dispatch("FIRST_TEAM_ERROR", curErrArr);
    },
...
<style >
#btnOne.v-item--active, #btnTwo.v-item--active, #btnThree.v-item--active {
   background-color: rgba(252, 44, 44, 0.904);
}
</style>