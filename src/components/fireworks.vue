// Thancks to the author of this code: https://github.com/dampion/Vue-fireworks
<template>
    <div
    class="canvasBox"
    v-if="`${totScore()}`"
    :style="runFireworks ? `height: ${canvasBoxHeight}; width: ${canvasBoxWidth};` : `display: none`">
...
<script>
  export default {
    ...
    methods: {
      // контроль видимости феерверка
      totScore(){
        let totalRoundSco = this.$store.getters.getTotalBigScore;
        if (totalRoundSco >= 200) {
          this.runFireworks = true
          return true
        } else {
          this.runFireworks = false
          return false
        }
      },
